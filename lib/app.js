...
function courseCard(course) { const saved = loadProgress(course.id), questionCount = course.questionCount ?? course.questions ?? '—', complete = typeof questionCount === 'number' ? percent(Object.keys(saved.answers || {}).length, questionCount) : 0; return `<article class="card course-card" data-course-card="${esc(course.id)}"><div class="eyebrow">${esc(course.code || 'Certification')}</div><h3>${esc(course.title)}</h3><p class="subtle">${esc(course.description || 'Open this course workspace.')}</p><div class="meta"><span>${esc(questionCount)} questions</span>${course.provider ? `<span>${esc(course.provider)}</span>` : ''}</div><progress class="progress" max="100" value="${complete}" aria-label="${complete}% locally complete"></progress><div class="progress-label">${complete}% locally complete</div></article>`; }
