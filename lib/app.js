...
const app = {
  ...
  practiceFilter: 'all', mock: null, timer: null, pending: new Set(),
...
const usernameEmail = username => `${username}@users.certlab.invalid`;
...
function baseProgress() { return { version: 2, answers: {}, bookmarks: {}, streak: 0, migratedLegacy: false, updatedAt: null }; }
...
function saveProgress(render = true) {
...
async function go(route, replace = false) {
  ...
  const view = ['dashboard','notes','practice','flashcards','mock','review'].includes(route.view) ? route.view : 'dashboard';
...
function courseCard(course) { const saved = loadProgress(course.id), questionCount = course.questionCount ?? course.questions ?? '—', complete = typeof questionCount === 'number' ? percent(Object.keys(saved.answers || {}).length, questionCount) : 0; return `<article class="card course-card" data-course-card="${esc(course.id)}" style="--course-accent:${esc(course.accent || 'var(--accent)')}"><div class="eyebrow">${esc(course.code || 'Certification')}</div><h3>${esc(course.title)}</h3><p class="subtle">${esc(course.description || 'Open this course workspace.')}</p><div class="meta"><span>${esc(questionCount)} questions</span>${course.provider ? `<span>${esc(course.provider)}</span>` : ''}</div><div class="progress"><i style="width:${complete}%"></i></div><div class="progress-label">${complete}% locally complete</div></article>`; }
function renderDashboard() {
  const title = app.data.metadata?.title || app.course.title;
  const domains = app.data.domains.map(domain => { const qs = domainQuestions(domain.id), done = qs.filter(question => app.progress.answers[question.id]).length; return `<article class="card domain-card" data-domain="${esc(domain.id)}"><div class="eyebrow">${esc(domain.weight ? `${domain.weight}% exam weight` : `Domain ${domain.id}`)}</div><h3>${esc(domain.name)}</h3><p class="subtle">${esc(domain.description || `${qs.length} practice questions`)}</p><div class="progress"><i style="width:${percent(done,qs.length)}%"></i></div><div class="progress-label">${done}/${qs.length} answered</div></article>`; }).join('');
  return topbar(title, app.data.metadata?.description || app.course.description) + statsHtml() + `<section class="section-head"><div><h2>Study this course</h2><p>Choose a domain or continue where you left off.</p></div><button class="btn primary" data-view="practice">Continue practice</button></section><section class="grid domain-grid">${domains}</section><section class="section-head"><div><h2>Course catalog</h2><p>Switch among certification tracks without mixing progress.</p></div></section><section class="grid course-grid">${app.courses.map(courseCard).join('')}</section>`;
}
function renderNotes() { const cards = app.data.domains.map(domain => { const objectives = safeArray(domain.objectives).map(item => `<div class="objective"><b>${esc(typeof item === 'string' ? item : item.title ?? item.name ?? item.id ?? 'Objective')}</b>${typeof item === 'object' && (item.description || item.summary) ? `<p>${esc(item.description ?? item.summary)}</p>` : ''}</div>`).join('') || `<div class="objective"><b>Practice focus</b><p>Use the course questions tagged to this domain to validate your understanding.</p></div>`; return `<article class="card"><div class="eyebrow">${esc(domain.weight ? `${domain.weight}% exam weight` : `Domain ${domain.id}`)}</div><h2>${esc(domain.name)}</h2>${domain.description ? `<p class="subtle">${esc(domain.description)}</p>` : ''}<div class="objective-list">${objectives}</div><div class="question-actions"><span class="meta">${domainQuestions(domain.id).length} practice questions</span><button class="btn small" data-domain-practice="${esc(domain.id)}">Practice domain</button></div></article>`; }).join(''); return topbar('Domain notes','Review objectives, then validate each domain in practice.') + `<section class="grid">${cards}</section>`; }
function filteredQuestions() { return app.practiceFilter === 'all' ? app.data.questions : domainQuestions(app.practiceFilter); }
...
function renderPractice() { const all = filteredQuestions(); if (app.questionIndex >= all.length) app.questionIndex = 0; const question = all[app.questionIndex]; const options = [`<option value="all">All domains (${app.data.questions.length})</option>`, ...app.data.domains.map(domain => `<option value="${esc(domain.id)}" ${app.practiceFilter === domain.id ? 'selected' : ''}>${esc(domain.name)} (${domainQuestions(domain.id).length})</option>`)].join(''); return topbar('Practice questions','Answer explanations appear after you check an answer.') + `<section class="question-tools"><label class="field">Filter questions<select id="practice-filter">${options}</select></label><button class="btn small" data-action="shuffle-practice">Shuffle</button><span class="meta">${all.length} available</span></section><section class="section-head"><div><h2>${esc(domainName(question.domainId))}</h2></div></section>${questionHtml(question, app.questionIndex, all.length, 'practice')}`; }
...
function render() { if (!app.course || !app.data) return; const body = ({dashboard:renderDashboard,notes:renderNotes,practice:renderPractice,flashcards:renderFlashcards,mock:renderMock,review:renderReview}[app.view] || renderDashboard)(); $('#app').innerHTML = layoutHtml(body); bindEvents(); }
